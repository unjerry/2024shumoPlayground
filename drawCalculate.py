...
print(torch.dist(z[1][-1], model.P0[0]) + model.vec[:].shape[0] * model.dl2)
print((torch.dist(z[0][-1], model.P1[0]) + model.vec[:].shape[0] * model.dl) / model.dl)
print(
    (torch.dist(z[1][-1], model.P0[0]) + model.vec[:].shape[0] * model.dl2) / model.dl2
)
with open("result" + lable, "w") as file:
    ...
    file.write(
        f"B_total_distance:{torch.dist(z[1][-1], model.P0[0]) + model.vec[:].shape[0] * model.dl2}\n"
    )
    file.write(
        f"A_total_time:{(torch.dist(z[0][-1], model.P1[0]) + model.vec[:].shape[0] * model.dl) / model.dl}\n"
    )
    file.write(
        f"B_total_time:{(torch.dist(z[1][-1], model.P0[0]) + model.vec[:].shape[0] * model.dl2) / model.dl2}\n"
    )
