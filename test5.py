...
class M(nn.Module):  # 无人机飞行轨迹
    def __init__(self, n, p0, p1, dl, *args, **kwargs) -> None:
        ...
        self.P0 = nn.Parameter(p0, requires_grad=False)  # 无人机A的轨线上的插点
        self.P1 = nn.Parameter(p1, requires_grad=False)  # 无人机B的轨线上的插点
        self.dl = nn.Parameter(torch.tensor(dl), requires_grad=False)  # 无人机A的步长
        self.dl2 = nn.Parameter(torch.tensor(dl), requires_grad=False)  # 无人机B的步长
        self.theh = nn.Parameter(
            torch.tensor([torch.pi / 6]), requires_grad=True
        )  # 无人机A的起飞角度α0
        self.theh2 = nn.Parameter(  # 无人机A的起飞角度β0
            torch.tensor([torch.pi + torch.arcsin(torch.ones([]) / 7.0)]),
            requires_grad=True,
        )
        self.unit = nn.Parameter(torch.tensor([1.0, 0]), requires_grad=False)

    def forward(self):  # 由参数计算曲线插点
...
def cost_function(x, y):  # 计算总势能，势能最小即为等价泛函极值
    softplus = torch.nn.Softplus(beta=1000.0 / rt)  # 统一凸性势能函数
    end = torch.tensor([35.0 * rt, 0.0], device="cpu")
    end2 = torch.tensor([-50.0 * rt, 0.0], device="cpu")
    circ = torch.tensor([0.0, 0.0], device="cpu")
    dif = x[-1] - end
    dif2 = y[-1] - end2
    dist = torch.einsum("i,i->", dif, dif)  # F1距离部分
    # dist = 0
    ...
    for i in range(x.shape[0]):  # F3避免碰面部分
        # print(i)
        meet = y[i] - x[i]
        # print("x: ", meet, " end")
...
# 初始设定
p0 = torch.tensor([[-15.0 * rt, 0.0]], device="cpu")
...
for k in range(-10, -11, -1):
    Los = []
    print(k)
    p0 = torch.tensor([[-50 * rt, 0.0]], device="cpu")
    # model = torch.load("model")
    ...
    #     requires_grad=True,
    # )
    model.dl = nn.Parameter(torch.tensor(0.15 * rt), requires_grad=False)
    model.to("cpu")
    for i in range(30000):
        ...
        if (i + 1) % 500 == 0 or i == 0:  # 保存模型
            ...
            torch.save(Los, f"Los5-5000-15")
            torch.save(model, f"model5-5000-15")
