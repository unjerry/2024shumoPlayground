...
def cost_function(x, y):
    ...
    ans = dd + dd2
    # ans = 0
    for th in model.theh:
        ans += torch.abs(th)
    for th in model.theh2:
        ans += torch.abs(th)
    for p in x:
        # print(p)
        # # print(
    ...
    for i in range(x.shape[0]):
        ...
        #     (dist + dist2)
        #     * 0.1
        #     * softplus(torch.einsum("i,i->", meet, x[i]) / torch.dist(meet, circ))
        # )
        ans += (
            (dist + dist2)
            * 0.1
            * softplus(torch.einsum("i,i->", meet, -y[i]) / torch.dist(meet, circ))
        )
    ...
    if (i + 1) % 500 == 0 or i == 0:
        ...
        with open("latestparam", "w") as fo:
            for pr in model.parameters():
                # print(pr)
                fo.write(f"{pr}\n")
        torch.save(model.vec, "vecgai")
        torch.save(model.vec2, "vec2gai")
        torch.save(Los, "Losgai")
        torch.save(model, "modelgai")
