...
class M(nn.Module):
    def __init__(self, n, p0, dl, *args, **kwargs) -> None:
...
for i in range(100000):
    opt.zero_grad()
    z = model()
    loss = cost_function(z)
    loss.backward()
    opt.step()
    torch.set_printoptions(precision=4, linewidth=1000)
    print(
        f"{i}\t{z[-1]}\t{loss.tolist():.4f}\t{preloss.tolist():.4f}\t{(torch.abs(preloss-loss)).tolist():.4f}"
    )
    # if torch.abs(preloss - loss).tolist() < 1e-5 or loss.tolist() < 1e-4:
    #     break
    preloss = loss
    if (i + 1) % 1000 == 0:
        z = model().detach().cpu()
