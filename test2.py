        ...
        if (i + 1) % 500 == 0 or i == 0:
            ...
            with open("latestparam", "w") as fo:
                for pr in model.parameters():
                    # print(pr)
                    fo.write(f"{pr}\n")
            torch.save(model.vec, "3vec")
            torch.save(model.vec2, "3vec2")
            torch.save(Los, f"Los3{k}")
            torch.save(model, f"model3{k}")
